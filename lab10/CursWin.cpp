...
CursWin::CursWin(bool init) {
    InitCurses();

    upperLeftRow = 0;
    upperLeftCol = 0;
    lowerRightRow = LINES-1;
    lowerRightCol = COLS-1;
    isPopUp = false;
    restore = NULL;
    DrawBoundingBox('-','|','+');
    writeEnabled = true;
...
void CursWin::WriteAsciiImage(int r, int c, FILE* fIn, bool ref) {
    ...
    char isEof;
    ClearWin();
    GetLineWithInfo(line, isEof, fIn);
    curRowPos = r;
    curColPos = c;
    while (isEof != EOF) {
        Writes(line);
...
void CursWin::WriteText(char* txt, bool ref) {
    ...
    char blankWord[2] = "";
    if (((int)(strlen(txt)-1) + curColPos) > maxColPos) {
        temp = GetWord(temp,nextWord);
        if (((int)(strlen(nextWord)) + curColPos) > maxColPos) {
            char* t2 = &(nextWord[0]);
...
char* GetLineWithInfo(char buf[], char &lastread, FILE* fIn) {
    int pos = 0;
    char inch = fgetc(fIn);
    while ((inch != '\n') && (inch != EOF)) {
        buf[pos++] = inch;
